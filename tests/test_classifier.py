...
from pipeline.processors import classify
...
def test_classify_retorna_categoria_valida():
    ...
    with patch('pipeline.processors.classifier.client.chat.completions.create', return_value=mock_resp):
...
def test_classify_erro_api_retorna_outro():
    with patch('pipeline.processors.classifier.client.chat.completions.create', side_effect=Exception('timeout')):
        ev = {'titulo':'X','data_iso':'','local':'','descricao_bruta':''}
        r = classify(ev)
        assert r['categoria'] == 'Outro'
